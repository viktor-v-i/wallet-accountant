...
# Bank Accountant Agent (`bank/`)

A separate Railway service in this same repo that monitors Nordea bank account transactions via manual CSV export + Telegram upload.

## Architecture

- **bank/monitor.js** — Entry point. Initializes DB and starts the Telegram bot.
- **bank/commands.js** — All Telegram command handlers + CSV file upload handler.
- **bank/ingest.js** — Nordea CSV parser + deduplication + AI analysis pipeline.
- **bank/analyzer.js** — OpenRouter AI integration. Tags each transaction as `acceptable`, `questionable`, or `wasteful` and assigns a category.
- **bank/db.js** — PostgreSQL via `pg`. Tables: `bank_transactions`, `bank_merchants`.
- **bank/bot.js** — Telegram bot instance using `BANK_TELEGRAM_BOT_TOKEN`.

## Deployment

- **Platform:** Railway — same project (`agent-accountant`), new service (`bank-agent`)
- **Root directory:** `/bank` (set in Railway service settings)
- **Start command:** `npm start` → `node monitor.js`
- **Database:** Shared Postgres plugin from the same Railway project

## Environment Variables (set in Railway dashboard for bank-agent service)

| Variable | Description |
|---|---|
| `BANK_TELEGRAM_BOT_TOKEN` | New bot token from BotFather (separate from wallet-agent) |
| `BANK_TELEGRAM_CHAT_ID` | Your Telegram chat ID |
| `OPENROUTER_API_KEY` | Same as wallet-agent |
| `MODEL` | `moonshotai/kimi-k2.5` or any OpenRouter model |
| `DATABASE_URL` | Auto-injected by Railway from the shared Postgres plugin |

## CSV Format (Nordea export)

Tab-separated. Columns: `Bokföringsdag`, `Belopp`, `Avsändare`, `Mottagare`, `Namn`, `Rubrik`, `Saldo`, `Valuta`

- Dates: `2026/03/28` → normalized to `2026-03-28`
- Amounts: `-300,00` (comma decimal, negative = outgoing)
- Deduplication key: `date|amount|description` — safe to upload the same CSV multiple times

## AI Tagging

Each transaction is tagged by the AI as:
- `acceptable` — necessary/justified spending
- `questionable` — discretionary, may or may not be justified
- `wasteful` — clear overspending or impulse purchase

Edit the `SPENDING_CRITERIA` block in `bank/analyzer.js` to define your personal criteria.

## Telegram Commands

| Command | Description |
|---|---|
| Upload `.csv` | Parse, deduplicate, AI-analyze, send digest |
| `/today` | Today's transactions |
| `/recent` | Last 10 transactions |
| `/wasteful` | Wasteful transactions (last 30 days) |
| `/questionable` | Questionable transactions (last 30 days) |
| `/categories` | Visual spending breakdown by category |
| `/week` | AI summary of last 7 days |
| `/month` | AI summary of last 30 days |
| `/search <keyword>` | Search by merchant, description, or category |
...
1. Add handler in `bank/commands.js` inside `setupCommands()`
2. Add DB queries to `bank/db.js`
3. Add to `/help` handler
4. Push to GitHub — Railway auto-deploys
